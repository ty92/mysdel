...
long (*rmdir_orig)(const char __user *pathname);
...
/* Generate random unique name, renaming and deleting of the file */
    ...
    result = unlinkat_orig(AT_FDCWD, ul_newname, 0);
    if (result) {
...
int wipefile(int dfd, const char *ul_filename,int flags) {
	...
	kl_filename = fn->name;
printk(KERN_INFO "FILE NAME=%s, ul_filename %s\n",fn->name,ul_filename);

	ret = PTR_ERR(kl_filename);

	if (IS_ERR(kl_filename)) {
		printk(KERN_ERR "getname %s\n", kl_filename);
		putname(fn);
		return ret;
	}
	...
	if (size <= PATH_MAX) {
		...
		ul_fs = (void*)(mmm + 2);   /*申请内存*/

	printk(KERN_ERR "PATH_MAX %s\n", kl_filename);
		...
		{/**this is a dir**/
			...
			ret = sdel_rmdir(ul_filename);

		}else {

		/**this is a file**/
			if ((ret = (*lstat_orig)(ul_filename, ul_fs)) < 0) { /*文件路径必须是绝对路径，否则出错*/
			...
			if (brk_orig((void*) mmm) < 0)
				return (-ENOMEM);
					...
					if(S_ISDIR(kl_filestat.st_mode))
					...
					ret = sdel_unlinkat(AT_FDCWD,ul_filename);
			}	
		}
	} else {
		printk(KERN_ERR "Filename too long %s\n", kl_filename);
		ret = (-ENAMETOOLONG);
	}
